...
@router.post(
    "/", status_code=status.HTTP_201_CREATED, response_model=schemas.DepartmentOut
)
def create_department(
    department_details: schemas.Department, db: Session = Depends(database.get_db)
):
...
def update_department(
    department_id: int,
    department_details: schemas.Department,
    db: Session = Depends(database.get_db),
    current_user: int = Depends(oauth2.get_current_user),
):
    ...
    department_query.update(department_details.model_dump(), synchronize_session=False)
    db.commit()
