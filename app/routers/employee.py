...
def update_employee(
    ...
    if current_user.id != employee.user_id:
        raise HTTPException(
            ...
            detail="You have no permissions to update profile",
        )
    department = (
        db.query(models.Department)
        .filter(models.Department.id == employee_details.department_id)
    ...
    employee_query.update(**employee_details.model_dump(), synchronize_session=False)
    db.commit()
...
def delete_employee(
    ...
    if current_user.id != employee.user_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="You have no permissions to delete profile",
        )
